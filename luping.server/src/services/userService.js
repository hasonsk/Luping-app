...
const registerAdmin = async (data) => {
  ...
  const accessToken = generateAccessToken(savedAdmin._id, savedAdmin.role);
  const refreshToken = generateRefreshToken(savedAdmin._id, savedAdmin.role);
...
const registerUser = async (data) => {
    ...
    if (existingUser) {
        const err = new Error('User with this email already exists.');
        err.status = 409;
        err.detail = 'The provided email is already registered, please use another email.';
        throw err;
    }
...
const loginUser = async ({ email, password }) => {
    ...
    if (!user || !(await user.matchPassword(password))) {
        const err = new Error('Invalid email or password.');
        err.status = 401;
        err.detail = 'The provided email does not exist or the password is incorrect.';
        throw err;
    }
...
const sendCode = async (email) => {
    if (!email) {
        const err = new Error('Email is required');
        err.status = 400;
        err.detail = 'Please provide the email address associated with your account.';
        throw err;
    }
    const user = await User.findOne({ email });
    if (!user) {
...
const verifyCode = async (email, code) => {
    ...
    if (user.codeExpires < Date.now()) {
        const err = new Error('Code expired');
        err.status = 400;
        err.detail = 'The verification code has expired. Please request a new code.';
        throw err;
    }
    if (user.code !== code) {
        const err = new Error('Invalid code');
