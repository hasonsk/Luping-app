...
async function analyzePronunciation(parsedXml, targetText) {
    const result = {
        overallScore: parseFloat(parsedXml.xml_result.read_sentence[0].rec_paper[0].read_sentence[0].$.total_score),
    ...
    let recognizedText = '';
    if (parsedXml.xml_result.read_sentence[0].rec_paper[0].read_sentence[0].sentence) {
        const words = parsedXml.xml_result.read_sentence[0].rec_paper[0].read_sentence[0].sentence[0].word;
...
function getDpMessageMeaning(dpMessage) {
    switch (dpMessage) {
        ...
        case 32: return "Added";
        case 64: return "Readback";
...
const evaluatePronunciation = (audioFilePath, textToEvaluate) => {
    return new Promise((resolve, reject) => {
        const authUrl = getAuthUrl(IFLYTEK_API_URL, IFLYTEK_API_KEY, IFLYTEK_API_SECRET);
        ...
        ws.on('open', () => {
            ...
            const stream = fs.createReadStream(audioFilePath, { highWaterMark: 1280 }); // 1280 bytes per chunk (40ms at 16kHz, 16-bit, mono)
            let frameCount = 0;
        ...
        ws.on('message', async (data) => { // <--- Make this callback async
            const result = JSON.parse(data);
            // logger.info(`Received message: ${JSON.stringify(result)}`); // Log the raw response
            if (result.code !== 0) {
                reject(new Error(`IFLYTEK API error: ${result.message} (code: ${result.code})`));
                ws.close();
                return;
            }
            ...
            if (result.data && result.data.status === 2) {
                ...
                try {
                    ...
                    const decodedXml = Buffer.from(result.data.data, 'base64').toString('utf8');

                    const parsedXml = await parseStringPromise(decodedXml); // Parse the XML

                    const analysis = await analyzePronunciation(parsedXml, textToEvaluate);
